...
export function EntradaSalidadEmpleadosPage() {
  ...
  const filteredHoras = useMemo(() => {
    return horasList.filter((hora) => {
      const fullName =
        `${hora.Empleado.nombre} ${hora.Empleado.apellido}`.toLowerCase();
  ...
  const {
    currentData: pageHoras,
    currentPage,
    maxPage,
    jump,
  } = usePagination(filteredHoras, 6);
            ...
            <TableBody className="w-full bg-white">
              {pageHoras.map((hora, index) => (
                ...
                >
                  ...
                  <TableCell className="">{hora.Empleado.cedula}</TableCell>
